/**
 * Cloudflare Pages Function — GET /api/market
 *
 * Auto-deploys with the site (no manual Worker paste). Same-origin, so the
 * Market Today page just fetches "/api/market".
 *
 * Sources (all optional/graceful, never throws):
 *   - indices ........ CBOE delayed quotes                       [keyless]
 *   - gainers/losers .. FMP (if FMP_API_KEY) else Yahoo          [FMP recommended;
 *                       Yahoo often blocks Cloudflare IPs]
 ...
 *
 * Set FINNHUB_API_KEY and FMP_API_KEY in the Pages project:
 *   Workers & Pages -> globesec-website -> Settings -> Variables and Secrets.
 */
...
const UA = "Mozilla/5.0 (globesec-csp)";
...
function summarize(ix) {
  const w = (ix || []).filter((i) => i.change_pct != null);
...
export async function onRequest(context) {
  ...
  try {
    ...
    return json({
      ...
      indices: indices || [], gainers: gainers || [], losers: losers || [],
      earnings_week: earnings_week || [], news: headlines || [],
      has_finnhub: !!key, has_fmp: !!(env && env.FMP_API_KEY),
      debug: dbg,
    }, CACHE_SECONDS);
  } catch (e) {
    return json({ error: String(e), debug: dbg }, 30);
  }
}
